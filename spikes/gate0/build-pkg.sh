...
inside_build() {
  ...
  timeout 7200 chroot --userspec=1000:1000 "$ROOT" /usr/bin/env -i PATH=/usr/bin HOME=/build-home LANG=C.UTF-8 \
    MAKEFLAGS="-j$(nproc)" PKGDEST=/build/out bash -c "cd $b && makepkg --noconfirm --cleanbuild" \
  ...
  case ${1:-} in
    ...
    *)
      ...
      sudo unshare --mount --pid --fork --propagation private -- \
        env RAYTONE_NS=1 WORK="$WORK" EVID="$EVID" PKG_MIRROR="${PKG_MIRROR:-}" bash "$SCRIPT_BP" inside-build "$(basename "$src")" "$src"
